...
import { Card, Table, Button, Input, Modal, Icon, SkeletonRow, ConfirmationModal } from '../../../components'
...
const CMS = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await api.pages.delete(slugToDelete)
      success('Página eliminada')
      setPages(pages.filter(p => p.slug !== slugToDelete))
  ...
  return (
    <div className="admin-cms-page">
      ...
      <div className="cms-tabs">
        ...
        >
          <Icon name="fileText" size={16} className="mr-2" /> Páginas y Contenido
        </button>
        <button
          className={`cms-tab ${activeTab === 'design' ? 'active' : ''}`}
          onClick={() => setActiveTab('design')}
        >
          <Icon name="grid" size={16} className="mr-2" /> Diseño (Header/Footer)
        </button>
      </div>
        ...
        {showSkeleton ? (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ background: 'var(--bg-secondary)', borderBottom: '2px solid var(--border-color)' }}>
              {['TÍTULO', 'SLUG', 'SECCIÓN', 'LAST UPDATE', 'ACCIONES'].map(h => <th key={h} style={{ padding: '14px 16px', textAlign: 'left', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '0.05em', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>{h}</th>)}
            </tr></thead>
              ...
              <div className="design-grid" style={{ padding: '2rem', display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                {mergedDesign.map(layout => (
                  <div key={layout.slug} className="layout-card" style={{ border: '1px solid var(--border-color)', padding: '1.5rem', borderRadius: '8px' }}>
                    ...
                    <Button variant="secondary" onClick={() => handleOpenModal(layout)} style={{ width: '100%' }}>Editar Diseño</Button>
                  </div>
                ))}
                <div className="info-box" style={{ gridColumn: '1 / -1', marginTop: '1rem', padding: '1rem', background: 'rgba(59, 130, 246, 0.1)', borderRadius: '8px' }}>
                  <p><Icon name="info" size={16} className="mr-2" /> <strong>Nota:</strong> El diseño usa el contenido HTML para renderizar elementos dinámicos. Ten cuidado al borrar etiquetas estructurales.</p>
                </div>
              </div>
            )}
          </>
      ...
      <ConfirmationModal
        ...
        title="Eliminar Página CMS"
        message={`¿Estás seguro de que deseas eliminar permanentemente la página "/${slugToDelete}"? Esta acción no se puede deshacer.`}
