...
import { HardDrive, FolderKey, Loader, AlertTriangle } from 'lucide-react';
...
const SourceList = ({ sources, loading, error, onSelect, onRefresh }) => {
    ...
    if (error) {
        return (
            <div className="p-4 bg-red-900/20 border border-red-800 rounded-lg flex items-center gap-3 text-red-200">
                ...
                <span>{error}</span>
                <button onClick={onRefresh} className="ml-auto text-sm underline hover:text-white">
                    Reintentar
                </button>
            </div>
        );
    ...
    if (sources.length === 0) {
        return (
            <div className="text-center p-8 border-2 border-dashed border-gray-700 rounded-lg text-gray-500">
    ...
    return (
        <div className="space-y-3">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-white">Fuentes Disponibles</h3>
                <button
                    onClick={onRefresh}
            ...
            <div className="grid gap-3">
                {sources.map((source) => (
                    <div
                        key={source.id}
                        className="bg-gray-800 hover:bg-gray-750 border border-gray-700 rounded-lg p-4 transition-all"
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-400 font-mono mt-1">{source.path}</p>

                                {source.preview_files && source.preview_files.length > 0 && (
                                    <div className="mt-3">
                                        <p className="text-xs text-gray-500 mb-1">Archivos detectados:</p>
